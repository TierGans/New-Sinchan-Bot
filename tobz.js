...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = ["6282134580805@c.us","628876729780@c.us"]
        ...
        const vhtearkey = 'SinchanBOT' // https://api.vhtear.com
        const barbarkey = 'pJ6xKSMPCT8QMcvBzGFY' // https://mhankbarbar.herokuapp.com/api
        ...
        // FUNCTION
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Sinchan!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Sinchan!', id)
        ...
        case '#banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sinchan!', id)
...
${desc}`)
		...
		case '#slap':
            arg = body.trim().split(' ')
            const porsen = author.replace('@c.us', '')
            await tobz.sendGiphyAsSticker(from, 'https://media.giphy.com/media/S8507sBJm1598XnsgD/source.gif')
            tobz.sendTextWithMentions(from, '@' + porsen + ' slapped ' + arg[1])
            break
        case '#hug':
            arg = body.trim().split(' ')
            const porson = author.replace('@c.us', '')
            await  tobz.sendGiphyAsSticker(from, 'https://media.giphy.com/media/uMfodOm1hDUdUz9iIF/source.gif')
            tobz.sendTextWithMentions(from, '@' + porson + ' Hug ' + arg[1])
            break
        case '#kiss':
            arg = body.trim().split(' ')
            const persen = author.replace('@c.us', '')
            await  tobz.sendGiphyAsSticker(from, 'https://media.giphy.com/media/lprIpUuIQ7kVwBI0pW/souce.gif')
            tobz.sendTextWithMentions(from, '@' + persen + ' has kissed ' + arg[1])
            break
		case '#kill':
            arg = body.trim().split(' ')
            const pirsin = author.replace('@c.us', '')
            await  tobz.sendGiphyAsSticker(from, 'https://media.giphy.com/media/2bajZIQ94gW0vRdAFr/souce.gif')
            tobz.sendTextWithMentions(from, '@' + pirsin + ' has killed ' + arg[1])
            break
        case '#kapankah':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        ...
        case '#owner':
        case '#creator':
            tobz.sendContact(chatId,`6282134580805@c.us`)
            tobz.reply(from, 'itu nomor tierkun >< ,creatorku', id)
			tobz.sendContact(chatId,`628876729780@c.us`)
            tobz.reply(from, 'itu nomor Xyro >< ,creatorku juga', id)
            break
        // ON OFF
        case '#nsfw':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        ...
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Sinchan!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
        ...
        case '#pantun':
            ...
            .then(body => {
                ...
                tobz.reply(from, randompantun.replace(/tobz-line/g,"\n"), id)
            })
            .catch(() => {
                tobz.reply(from, 'Ada yang Error!', id)
            })
...
*Text : ${swrt}*`
        ...
        case '#distord':
            ...
            tobz.reply(from, config.msg.waitConvert(_mimetype.replace(/.+\//, ''), 'mp3', '⚠ WARNING ⚠\n🔇 Tau lah :v'), id)
            ...
            tobz.sendFile(from, baseURI(distord, 'audio/mp3'), 'distorted.mp3', '', id)
        } else if (isQuotedVideo) {
            // // Bantuin ffmpeg nya :')
            // // biar bisa video filter sama audio filter
            tobz.reply(from, config.msg.waitConvert('mp4', 'mp4', '⚠ WARNING ⚠\n🔇 Tau lah :v'), id)
            ...
            tobz.sendFile(from, baseURI(distord, 'video/mp4'), 'distorted.mp4', '', id)
        }
        break
        case '#bass':
        if (quotedMsgObj) {
            let encryptMedia
            const replyOnReply = await tobz.getMessageById(quotedMsgObj.id)
            ...
            if (encryptMedia.animated) {
                tobz.reply(from, config.msg.waitConvert('webp', 'mp4', 'Kebalikan dari gifstiker'), id)
                ...
                tobz.sendRawWebpAsSticker(from, sticker.toString('base64'), true)
            } else tobz.sendFile(from, baseURI(mediaData, _mimetype), `file.${_mimetype.replace(/.+\//, '')}`, ':)', encryptMedia.id)
        } else tobz.reply(from, config.msg.noMedia, id)
        break
        case '#cuaca':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case '#getvideo':
            ...
            if (quotedMsg && quotedMsg.type == 'image') {
                ...
                tobz.reply(from, mess.wait, id)
                const mhankyt45 = await fetch(`http://mhankbarbar.herokuapp.com/api/ytv?url=https://youtu.be/${pilur[args[1]]}&apiKey=${barbarkey}`)
                ...
                tobz.reply(from, mess.wait, id)
                const mhankyt45 = await fetch(`http://mhankbarbar.herokuapp.com/api/ytv?url=https://youtu.be/${args[1]}&apiKey=${barbarkey}`)
                if (!mhankyt45.ok) throw new Error(`Error Get Video : ${mhankyt45.statusText}`)
                const barbarytp45 = await mhankyt45.json()
                 if (barbarytp45.status == false) {
                    tobz.reply(from, `*Maaf Terdapat kesalahan saat mengambil data, mohon pilih media lain...*`, id)
		...
		case '#play': //silahkan kalian custom sendiri jika ada yang ingin diubah
            ...
            .then(async (res) => {
				...
				.then(async(res) => {
					if (res.status == 'error') return tobz.sendFileFromUrl(from, `${res.link}`, '', `${res.error}`)
					await tobz.sendFileFromUrl(from, `${res.thumb}`, '', `Lagu ditemukan\n\nJudul ${res.title}\n\nSabar lagi dikirim`, id)
					await tobz.sendFileFromUrl(from, `${res.link}`, '', '', id)
					.catch(() => {
						tobz.reply(from, `URL INI ${args[0]} SUDAH PERNAH DI DOWNLOAD SEBELUMNYA ..URL AKAN RESET SETELAH 60 MENIT`, id)
					})
				})
            })
            .catch(() => {
        ...
        case '#getmusic':
            ...
            try {
                if (quotedMsg && quotedMsg.type == 'image') {
                    ...
                    console.log(pilur[args[1]])
                    tobz.reply(from, mess.wait, id)
                    const mhankyt35 = await get.get(`http://mhankbarbar.herokuapp.com/api/yta?url=https://youtu.be/${pilur[args[1]]}&apiKey=${barbarkey}`).json()                    
                    ...
                    tobz.reply(from, mess.wait, id)
                    const mhankyt35 = await fetch(`http://mhankbarbar.herokuapp.com/api/yta?url=https://youtu.be/${args[1]}&apiKey=${barbarkey}`)
        ...
        case '#youtubesearch':
            ...
            break
	    case '#movie':
			if (args.length == 0) return tobz.reply(from, `Untuk mencari suatu movie dari website sdmovie.fun\nketik: ${prefix}movie judulnya`, id)
			rugaapi.movie((body.slice(7)))
			.then(async (res) => {
				if (res.status == 'error') return tobz.reply(from, res.hasil, id)
				await tobz.sendFileFromUrl(from, res.link, 'movie.jpg', res.hasil, id)
			})
			break
        ...
        case '#jurnalotaku':
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#jurnalotaku [query]*, Contoh : #jurnalotaku one piece')
            const jotaku = body.slice(13)
            tobz.reply(from, mess.wait, id)
            try {
                const jotakuc = await axios.get(`https://api.vhtear.com/jurnalotaku?query=${encodeURIComponent(jotaku)}&apikey=${vhtearkey}`)
                const jotakun = jotakuc.data.result.result
                let xixixi = `*Hasil Pencarian : ${jotaku}*\n`
                for (let i = 0; i < jotakun.length; i++) {
                    xixixi += `\n═════════════════\n\n*Judul* : ${jotakun[i].title}\n*Kategori* : ${jotakun[i].category}\n*Tanggal* : ${jotakun[i].date}\n*Link* : ${jotakun[i].link}\n`
                }
                await tobz.sendFileFromUrl(from, jotakun[0].img, 'jurnal.jpg', xixixi, id)
            } catch (err) {
                    console.log(err)
                    await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Jurnal tidak ditemukan')
                    tobz.sendText(ownerNumber, 'Jurnal Otaku Error : ' + err)
            }
            break
        case '#brainlysearch':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            ...
            await limitAdd(serial)
            if (args.length === 1) return tobz.reply(from, 'Kirim perintah *#brainlysearch [query]*, Contoh : *#brainlysearch Siapa itu Elaina?*')
...
➸ *Wilayah:* +${region_name}`
        ...
        case '#vote' :
			 ...
			 break
		case 'resetpoll':
             feature.adminpollreset(tobz, message, message.body.slice(10), pollfile, voterslistfile)
			 break
        case '#addcandidate':
             feature.addcandidate(tobz, message, message.body.slice(4), pollfile, voterslistfile)	
             break			 
		case '#user':
        if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
        if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
...
${sentence}`
        await tobz.sendTextWithMentions(from, shiptext)
        break
        case '#jadwalshalat':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        // ADMIN & OWNER
        case '#bc':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner Sinchan!', id)
            let msg = body.slice(4)
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) await tobz.sendText(ids, `[ SINCHANBOT BROADCAST ]\n\n${msg}`)
            }
        ...
        case '#omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Sinchan', id)
        ...
        case '#okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin Sinchan', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '#oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Sinchan', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '#oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Sinchan', id)
            await tobz.sendText(from,'SINCHAN DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '#leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
		...
		case '#closegroup':
		case '#groupclose':
            if (!isGroupMsg) return tobz.reply(from, 'Maaf Perintah Ini hanya dapat Digunakan Di dalam Group!!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Gagal Gan Perintah Ini Hanya bisa digunakan admin grup!', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Jadikan Bot admin Dulu Baru bisa gunakan fitur ini:)', id)
            await tobz.setGroupToAdminsOnly(groupId, true)
            await tobz.reply(from, 'Group Telah Di Tutup oleh admin')
            break
        case '#opengroup':
		case '#groupopen':
            if (!isGroupMsg) return tobz.reply(from, 'Maaf Perintah Ini hanya dapat Digunakan Di dalam Group!!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Gagal Gan Perintah Ini Hanya bisa digunakan admin grup!', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Jadikan Bot admin Dulu Baru bisa gunakan fitur ini:)', id)
            await tobz.setGroupToAdminsOnly(groupId, false)
            await tobz.reply(from, 'Group Telah Di Buka oleh admin')
            break
        case '#join':
        ...
        case '#getses':
            ...
            break
        case '#sinchanadmin':
            let admn = `This is list of Sinchan Admin\nTotal : ${adminNumber.length}\n`
	    ...
	    case '#reboot':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sinchan!', id)
            require('child_process').spawn(process.argv[0], process.argv.slice(1), { detachable: true, env: process.env })
            process.exit()
            break
        case '#addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sinchan!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin Sinchan >< !', id)
                }
            break
        case '#deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sinchan!', id)
                ...
                fs.writeFileSync('./lib/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin Sinchan T_T !', id)
            break
        case '#block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Sinchan!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '#unblock':
            ...
            break
        case '#ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Sinchan!', id)
                for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '#help':
            ...
            break
        case '#sinchangroup':
            tobz.reply(from, `Link Group Sinchan : https://chat.whatsapp.com/Igh2CFotKTQCnUpbaO7zvL\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#kerangmenu':
            ...
            break
		case '#edumenu':
            tobz.sendText(from, educmd)
			break
        case '#downloadmenu':
            tobz.sendText(from, downloadcmd)
            break
		case '#ngalimmenu':
           	tobz.sendText(from, ngalimcmd)
            break	
        case '#othermenu':
        ...
        case '#iklan':
            ...
            break
        case '#adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Sinchan', id)
        ...
        case '#snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Sinchan, Link Ada Di Bio atau Bisa Mengetik #sinchangroup\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
            if (command.startsWith('#')) {
